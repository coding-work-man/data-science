...
##  2014580    13637085
```
在这个例子里, 如果我们选取新的样本, 样本均值仅有2.5%的几率低于2,014,580. 类似的, 样本均值仅有2.5%的可能性会大于13,537,085(因为有97.5%的数值低于样本均值的分布).
...
我们的神秘样本的均值是 816,371. 但问题是, 这是一个美国各州的样本还是其他? 仅有这一个样本很难回答这个问题. 我们首先会观察到样本中含有比Kansas, Utah, Nebraska 和其他几个州都要多的人数. 从上文的人口数据的分布我们也可以看出很多州的人数都很小. 多亏了我们在上文中举得例子使得我们可以更好的进行比较. 我们有样本均值的分布并且我们可以很公平的拿新的样本均值与之对比, 新的均值超出了样本分布的极限区域, 也就是低于2.5%或高于97.5%. 故我们的神秘样本看起来不像是美国各州的取样.


既然这样, 我们可以清楚的看到816,371处在样本分布的极低区域. 回想我们使用quantile()函数式我们发现仅2.5%的样本均值低于2,014,580.
...
##      1410883     16792211
```
quantile()函数显示仅有0.5%的杭奔均值低于1,410,883. 因此我们的神秘样本的均值816,371假如是美国各州的采样, 那么必然是个小概率事件. 从这一点我们可以暂时性的但是基于有力的统计证据来推测, 我们的神秘样本**不是**美国各种的采样. 神秘样本的均值比美国各州样本的均值太多.
...
这是所有统计推断的基础. 你构建一个比较分布并且取走极限空间中的值,然后你用新获取到的数据与已知分布进行比较看新样本是否会落入极限空间. 如果新样本符合上述规律, 那么你可以暂时认为新样本是从其他数据源获取到的.

如果你有一些困惑那么就跟着心走. 上一段话代表了四五百年的数学发展.在没有像R这样的可以创造和分析真实样本分布的工具之前, 上述的绝大多数材料仅是一系列的公式和证明. 在后续的章节里我们会回到用户证明上述描述的特定统计规程. 但是现在我们仅需要几下其他三条信息.


第一, 我们使用mean()函数查看样本均值的分布和使用hist()函数查看它的形状, 但是我们从不量化分布的传播.
```{r}
sd(SampleMeans)

## 3037318
```
这里显示样本分布的标准方差. 统计学家称之为"均值的标准错误". 这个拗口的术语有另外一个清晰但较长的解释: 人口采样的样本分布均值的标准差. 不幸的是, 统计学家并非全都知道. 当我们看一个分布并且分布中的每个点都代表一个样本(例如, 一个均值), 那么标准方差就可以认为是标准差.


第二, 发现标准差其实不需要构造一个相同的样本分布均值.事实上, 原始数据的标准方差和标准差符合一定的线性关系:
```{r}
sd(USstatePops$V1)/sqrt(5)

## 3051779
```
上述公式表示用原始数据的标准偏差除以样本大小的平方根. 在上文中当我们使用replicate()和sample()命令创建SampleMeans向量是, 我们使用的样本大小n=5. 这就是为什么你在上文中看到sqrt(5)而不是sqrt()的原因. 在R或者其他软件中sqrt()是"square root"的简写, 而不是你预期的"squirt". 因此如果你有一系列数据并且你想计算他们的标准偏差,  你可以计算分布的标准差(每一个都有相同的样本大小), 除以样本大小的平方根. 你可能发现通过使用这种方法得出的结果会比直接从样本分布中得到的结果大一些, 但是这种差异没有任何意义(仅是因为分布的随机性导致). 另一个你会注意到的事情是样本空间越大, 标准差越小. 这对于样本的选择有很重要的法则: 样本越大越好.


最后是另外一个简便方法. 我们发现创建样本分布的97.5%的分割点并且使用quantile()函数计算真实的分割. 我们同样可以使用均值和标准差计算分割点. 均值减去2倍标注差就是2.5%的分割点, 均值加上2倍标准差就是97.5%的分割点.
```{r}
...
你会发现这个数值与我们使用quantile()函数计算全部分布的数值有差别. 这种差异由我们构建的分布的随机性导致. 上述结果仅是基于统计证明的估计值, 而我们创建的SampleMeans 数据集仅是我们能创建的最大的接近无线的集合. 我们可以很容易的通过使用更大的样本空间大小和添加更多的样本分布复制品来降低两种方法之间的矛盾.


总结一下, 我们使用了一个有51个美国各州人口数据的数据点的和一些R的知识创建了一个样本分布均值, 我们学到了以下知识:
- 持续运行一个统计过程多次你就可以得到一致的规律.
- 计算大样本的均值和绘制直方图可以展示样本均值是均匀分布并且分布中心与原始数据的均值接近.
- 样本均值的分布可以用来进行比较. 通过在极低的区域和极高的区域使用分割点, 例如2.5%和97.5%, 我们可以使用它与新获取的样本进行比较.
- 如果我们获得新的样本均值, 并且我们发现它掉入我们设定的极限区域, 我们可以暂时认为新样本与之前的数据来自不同数据源.
- 另外一种更精确的在计算标注差的方法是使用原始数据的标准偏差.


目前为止我们仍不失统计学家, 但是建立在样本分布上的推理过程是统计推理的核心. 假如你理解了本章的内容, 你已经迈向成为一个称职的应用统计用户的道路上走上了坚实的一步. 




本章测试
收集一个包含至少20个数据点的样本并且构造样本分布. 计算标注差和并且使用标准差计算2.5%和97.5%的分布分割点. 你收集的数据点必须代表相同数量的现象. 例如, 你可以手机20本教科书的价格, 或者统计20段文字中的单词数量.
